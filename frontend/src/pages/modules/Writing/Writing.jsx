...
function Writing() {
  ...
  const lessons = lessonsData.writing || []

  useEffect(() => {
    // Load completed lessons from localStorage
    const completed = JSON.parse(localStorage.getItem('completedLessons') || '[]')
    const writingCompleted = completed.filter(key => key.startsWith('writing-'))
    setCompletedLessons(writingCompleted)
    ...
    const loadFromBackend = async () => {
      try {
        ...
        if (token) {
          const response = await fetch('http://localhost:8000/api/v1/lesson-progress/writing', {
            headers: {
  ...
  const isLessonCompleted = (lessonId) => {
    // lessonId already contains the module prefix (e.g., 'writing-1')
  ...
  return (
    <div className={styles.writingModule}>
      <div className={styles.header}>
        <div className={styles.headerContent}>
          <div className={styles.moduleIcon}>✍️</div>
          <div>
            <h1>Writing Module</h1>
            <p>Improve your writing skills for Task 1 and Task 2</p>
          </div>
        </div>
