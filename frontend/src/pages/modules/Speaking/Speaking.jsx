...
function Speaking() {
  ...
  const lessons = lessonsData.speaking || []

  useEffect(() => {
    // Load completed lessons from localStorage
    const completed = JSON.parse(localStorage.getItem('completedLessons') || '[]')
    const speakingCompleted = completed.filter(key => key.startsWith('speaking-'))
    setCompletedLessons(speakingCompleted)
    ...
    const loadFromBackend = async () => {
      try {
        ...
        if (token) {
          const response = await fetch('http://localhost:8000/api/v1/lesson-progress/speaking', {
            headers: {
  ...
  const isLessonCompleted = (lessonId) => {
    // lessonId already contains the module prefix (e.g., 'speaking-1')
  ...
  return (
    <div className={styles.speakingModule}>
      <div className={styles.header}>
        <div className={styles.headerContent}>
          <div className={styles.moduleIcon}>🗣️</div>
          <div>
            <h1>Speaking Module</h1>
            <p>Build confidence in speaking with structured practice</p>
          </div>
        </div>
