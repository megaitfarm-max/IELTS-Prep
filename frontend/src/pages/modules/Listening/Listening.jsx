...
function Listening() {
  ...
  const lessons = lessonsData.listening || []

  useEffect(() => {
    // Load completed lessons from localStorage
    const completed = JSON.parse(localStorage.getItem('completedLessons') || '[]')
    const listeningCompleted = completed.filter(key => key.startsWith('listening-'))
    setCompletedLessons(listeningCompleted)
    ...
    const loadFromBackend = async () => {
      try {
        ...
        if (token) {
          const response = await fetch('http://localhost:8000/api/v1/lesson-progress/listening', {
            headers: {
          ...
          if (response.ok) {
            ...
            const merged = [...new Set([...listeningCompleted, ...backendCompleted])]
            setCompletedLessons(merged)
            // Update localStorage with merged unique values
            const allCompleted = JSON.parse(localStorage.getItem('completedLessons') || '[]')
            const otherModules = allCompleted.filter(key => !key.startsWith('listening-'))
  ...
  const isLessonCompleted = (lessonId) => {
    // lessonId already contains the module prefix (e.g., 'listening-1')
  ...
  return (
    <div className={styles.listeningModule}>
      <div className={styles.header}>
        <div className={styles.headerContent}>
          <div className={styles.moduleIcon}>🎧</div>
          <div>
            <h1>Listening Module</h1>
            <p>Enhance your listening skills with authentic audio materials</p>
          </div>
        </div>
      ...
      <div className={styles.lessonGrid}>
        {lessons.map((lesson, index) => {
          ...
          <Link 
            key={lesson.id} 
            to={`/lessons/listening/${lesson.id}`} 
